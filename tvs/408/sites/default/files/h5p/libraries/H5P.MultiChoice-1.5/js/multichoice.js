...
//   questionPoints: 10 //default number of points for getting this question right
...
H5P.MultiChoice = function(options, contentId, contentData) {
    ...
    $('input', $myDom).change(function () {
      ...
      if (params.behaviour.singleAnswer) {
        ...
        if (params.answers[num].correct) {
          score = Number(params.behaviour.questionPoints);
        } else {
          score = 0;
        }
  ...
  this.getMaxScore = function () {
    return Number(params.behaviour.questionPoints);
  };
  ...
  this.showCheckSolution = function () {
    ...
    if (score === max) { scoreText = "Correct.  ";}
  ...
  this.getScore = function() {
    ...
    for (var i = 0; i < params.answers.length; i++) {
        var choice = params.answers[i];
        if ((choice.correct === false) && (params.userAnswers.indexOf(i) > -1)) {
            allCorrect = false;
        }
        else if ((choice.correct) && (params.userAnswers.indexOf(i) === -1)) {
            allCorrect = false;
        }
    }
      
    if (allCorrect === false) { return 0; }
