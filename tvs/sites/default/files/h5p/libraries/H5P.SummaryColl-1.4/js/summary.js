    ...
    try {
        if (H5PIntegration.contents[Object.keys(H5PIntegration.contents)].library.toLowerCase().indexOf("h5p.presentation") === 0) {
            this.addButton('continue-button', "Continue", function () {
                if (H5P.instances[0].lastAnswerGiven) {
                    var lastAnswer = H5P.instances[0].lastAnswerGiven;
                    var allSummaries = H5P.instances[0].slides[H5P.instances[0].currentSlideIndex].elements[0].action.params.summaries[0].summary;
                    
                    for (var j = 0; j < allSummaries.length; j++) {
                        if (allSummaries[j].text.toLowerCase() === lastAnswer.toLowerCase()) {
                            var slideDest = allSummaries[j].branchOnSelectedOptions;
                            if ((slideDest) && (slideDest < H5P.instances[0].slides.length)) {
                                H5P.instances[0].jumpToSlide(slideDest - 1);
                                break;
                            }
                            else {
    ...
    for (var i = 0; i < that.summaries.length; i++) {
      ...
      for (var j = 0; j < that.summaries[i].summary.length; j++) {
        ...
        elements[i].summaries[j] = {
          id: c++,
          text: that.summaries[i].summary[j].text,
          branchSlide: that.summaries[i].summary[j].branchOnSelectedOptions
        };
      }
      ...
      for (var j = 0; j < element.summaries.length; j++) {
        ...
        $node.click(function() {
          ...
          that.actionData.answerProvided = this.innerText.replace("<div>","").replace("</div>","").replace("\n","");
            that.parent.lastAnswerGiven = that.actionData.answerProvided;
