H5P.InteractiveVideo = (function ($, EventDispatcher, DragNBar, Interaction) {
  ...
  function InteractiveVideo(params, id, contentData) {
    ...
    if ((self.options.video.files === undefined) && (params.sources) && (params.sources.length > 0)) {
        self.options.video.files = params.sources;
    }
  ...
  InteractiveVideo.prototype.addSliderInteractions = function () {
    // Remove old dots
    if ((this.interactions.length > 0) && ((this.video.getDuration() > 0) ||  (navigator.userAgent.indexOf('Android') > -1))) {
  ...
  InteractiveVideo.prototype.addBookmarks = function () {
    if ((this.options.assets.bookmarks !== undefined) && ((this.video.getDuration() > 0) ||  (navigator.userAgent.indexOf('Android') > -1))) {
        this.controls.$bookmarksContainer.children().remove();
        if (typeof(this.controls.$bookmarksList) !== "undefined") {
            this.controls.$bookmarksList.children().remove();
        }
  ...
  InteractiveVideo.prototype.resize = function () {
    ...
    if (fullscreenOn) {
      ...
      if (videoHeight + controlsHeight <= containerHeight) {
        ...
        width = height * ratio;
        if ((H5PIntegration.contents !== undefined) &&  (H5PIntegration.contents[Object.keys(H5PIntegration.contents)].library.toLowerCase().indexOf("h5p.mle") === 0)) {
            height = "auto";
        }
        this.$videoWrapper.css({
      ...
      this.video.trigger('resize');
        ...
        if (window.location.href.indexOf("/h5p/embed/") > -1) {
  ...
  InteractiveVideo.prototype.resizeMobileView = function () {
    ...
    var widthToEmThreshold = 24;
    var ivWidth = this.$container.width();
    if (ivWidth === 0) {
      ivWidth = (screen.width);
      if (ivWidth === 0) {
          ivWidth = (screen.availWidth);
          if (ivWidth === 0) {
              ivWidth = 900;
          }  
      }
    ...
    if (widthToEmRatio < widthToEmThreshold) {
      ...
      if (!this.isMobileView) {
        ...
        if ((/(13D15)/g.test( navigator.userAgent ) !== true) && (window.location.href.indexOf("showpad") === -1)) {
  ...
  InteractiveVideo.prototype.timeUpdate = function (time) {
    ...
    if (time > 0) {
      try {          
        ...
        var duration = self.video.getDuration();
        var newtime = InteractiveVideo.humanizeTime(duration);
        self.controls.$totalTime.html(newtime);
        self.controls.$slider.slider('option', 'max', duration);
          
        if ((navigator.userAgent.indexOf('Android') > -1) && (duration > 0) && (!androidFix)) {
            androidFix = true;
            self.oneSecondInPercentage = (100 / self.video.getDuration());
              
            self.addSliderInteractions();
            self.addBookmarks();
        }
  ...
  InteractiveVideo.prototype.getUsersScore = function() {
    ...
    for (var i = 0; i < this.interactions.length; i++) {
      if (typeof(this.interactions[i].getScore) !== "undefined") {
        score += this.interactions[i].getScore();
      }
    }
  ...
  InteractiveVideo.prototype.getUsersMaxScore = function() {
    ...
    for (var i = 0; i < this.interactions.length; i++) {
      if (typeof(this.interactions[i].getMaxScore) !== "undefined") {
        maxScore += this.interactions[i].getMaxScore();
      }
    }
