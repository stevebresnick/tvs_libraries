...
function setupListeners() {
    if (H5P.jQuery("#pdfFrame").contents().children().find("#goBtn").length > 0) {
        H5P.jQuery("#pdfFrame").contents().children().find("#goBtn")[0].addEventListener('change', handleSelectListChanges);
    }
    else {
        window.setTimeout(setupListeners,2000);
    }
}

window.setTimeout(setupListeners,2000);

function handleSelectListChanges(e) {
