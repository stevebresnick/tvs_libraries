...
H5P.ExportableTextArea.Exporter = (function _eta_exporter_internal() {
  ...
  this.run = function (slides, elements) {
      ...
      var thisExportText = this.createDocContent(slides, elements);
      
      var element = document.createElement('a');
      element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(thisExportText));
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      for (var j = 0; j < elements[i].length; j++) {
        ...
        if (element.libraryInfo && element.libraryInfo.machineName === 'H5P.ExportableTextArea') {
          ...
          slideHtml[element.index] = element.header + '\r\n' + input + '\r\n\r\n';
