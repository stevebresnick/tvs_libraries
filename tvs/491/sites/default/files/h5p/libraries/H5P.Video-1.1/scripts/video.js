  ...
  function Video(parameters, id) {
    ...
    self.attach = function ($container) {
      ...
      if (self.appendTo !== undefined) {
        ...
        if (sources.length) {
            if (sources[0].path.indexOf("youtube") > -1) {
                var availableHeight = screen.availHeight + "px";
                var iFrameSrc = '<iframe frameborder="0" allowfullscreen width="100%" height="' + availableHeight + '" src="';
                iFrameSrc += sources[0].path.replace("watch?v=","embed/");
                iFrameSrc += '"></iframe>';
                $container.html(iFrameSrc);
            }
            else {
