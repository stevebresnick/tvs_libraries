...
H5P.SingleChoiceSet = (function ($, Question, SingleChoice, SolutionView, ResultSlide, SoundEffects) {
  ...
  SingleChoiceSet.prototype.handleQuestionFinished = function (data) {
    ...
    var lastSelected = self.choices[self.choices.length - 1]
      ...
      for (var i = 0; i < lastSelected.children().length; i++) {
          if (lastSelected.children()[i].className === "h5p-sc-label") {
              if ((self.actionData === undefined) || (self.actionData === null)) {
                    self.actionData = new Object;  
              }
              self.actionData.answerProvided = lastSelected.children()[1].innerText.replace("<div>","").replace("</div>","").replace("\n","");
          }
      }
    ...
    if (data.correct) {
      self.setScore(Number(this.options.behaviour.questionPoints));
    }
    else {
        self.setScore(0);
    }
  ...
  SingleChoiceSet.prototype.setScore = function (score, noXAPI, timeout) {
    ...
    var skipXAPIUpdate = false;
    if (score === -1) { score = self.getScore(); skipXAPIUpdate = true; }
    ...
    var showFeedback = function () {
      var maxScore = Number(self.options.behaviour.questionPoints);
    
      var scoreText = "Incorrect.  ";
      if (score === maxScore) { scoreText = "Correct.  ";}
        
      self.setFeedback(scoreText,
        score, maxScore);
      if (score === maxScore) {
      ...
      if (skipXAPIUpdate === false) {
        self.triggerXAPIScored(Number(score), Number(maxScore), 'answered');  
  ...
  SingleChoiceSet.prototype.getScore = function () {
    if (this.results.corrects > 0) {
        return Number(this.options.behaviour.questionPoints);
    }
    else {
        return 0;
    }
  ...
  SingleChoiceSet.prototype.resetTask = function () {
    var self = this;
    H5P.jQuery(".h5p-question-content")[0].style.height="auto";
